...
class TestLoginUserPositive:
    ...
    def test_positive_login_user(self, get_base_url, data):
        response = requests.post(f'{get_base_url}api/login',
                                 data=data
                                 )

        assert response.status_code == 200, 'Wrong status code, expected - 200'
        # Check key token in response
...
class TestLoginUserNegative:
    ...
    def test_negative_login_user_without_password(self, get_base_url, data):
        response = requests.post(f'{get_base_url}api/login',
                                 data=data
                                 )
    ...
    def test_negative_login_user_without_email(self, get_base_url, data):
        response = requests.post(f'{get_base_url}api/login',
                                 data=data
                                 )
